...
//#include "cluster/ClusteringService_m.h"
...
void ClusterService::indicate(const btp::DataIndication& ind, cPacket* packet, const NetworkInterface& net)
{
    ...
    /*
        auto clusterMessage = check_and_cast<const PoliceClearLane*>(packet);

    const std::string id = mVehicleController->getVehicleId();
    auto& vehicle_api = mVehicleController->getTraCI()->vehicle;
    if (vehicle_api.getRoadID(id) == clusterMessage->getEdgeName()) {
        if (vehicle_api.getLaneIndex(status) != clusterMessage->getLaneIndex()) {
            print("Vehicle received a message");
        }
    */
...
void ClusterService::initialize()
{
	...
	m_self_msg = new cMessage("Cluster Service");
...
void ClusterService::trigger()
{
	...
	for (auto channel : mco.allChannels(example_its_aid)) {
		...
		if (network) {
			...
			cPacket* packet = new cPacket("Cluster Service Packet");
